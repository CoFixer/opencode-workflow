...
User submits prompt
    ↓
.opencode/settings.json registers hook
    ↓
...
Outputs formatted message to stdout
    ↓
stdout becomes context for OpenCode (injected before prompt)
    ↓
OpenCode sees: [skill suggestion] + user's prompt
```

### Key Points

- **Exit code**: Always 0 (allow)
- **stdout**: → OpenCode's context (injected as system message)
- **Timing**: Runs BEFORE OpenCode processes prompt
- **Behavior**: Non-blocking, advisory only
- **Purpose**: Make OpenCode aware of relevant skills
...
OpenCode sees this output as additional context before processing the user's prompt.
...
```
OpenCode calls Edit/Write tool
    ↓
.opencode/settings.json registers hook (matcher: Edit|Write)
    ↓
...
ELSE:
  Exit with code 0 (ALLOW)
    ↓
IF BLOCKED:
  stderr → OpenCode sees message
  Edit/Write tool does NOT execute
  OpenCode must use skill and retry
IF ALLOWED:
...
- **Exit code 2**: BLOCK (stderr → OpenCode)
- **Exit code 0**: ALLOW
...
OpenCode receives this message and understands it needs to use the skill before retrying the edit.
...
| Exit Code            | stdout      | stderr       | Tool Execution | OpenCode Sees    |
| -------------------- | ----------- | ------------ | -------------- | -------------- |
| 0 (UserPromptSubmit) | → Context   | → User only  | N/A            | stdout content |
| 0 (PreToolUse)       | → User only | → User only  | **Proceeds**   | Nothing        |
| 2 (PreToolUse)       | → User only | → **OpenCode** | **BLOCKED**    | stderr content |
...
1. **Only way** to send message to OpenCode from PreToolUse
2. stderr content is "fed back to OpenCode automatically"
3. OpenCode sees the block message and understands what to do
...
Prevent repeated nagging in the same session - once OpenCode uses a skill, don't block again.

### State File Location

`.opencode/hooks/state/skills-used-{session_id}.json`
...
- If OpenCode doesn't use the skill but makes a different edit, it won't block again
- Trust that OpenCode follows the instruction
