...
```bash
cat .opencode/skills/skill-rules.json | jq .
```
...
```bash
ls .opencode/hooks/state/
cat .opencode/hooks/state/skills-used-{session-id}.json
```
...
```bash
rm .opencode/hooks/state/skills-used-{session-id}.json
```
...
```bash
cat <<'EOF' | npx tsx .opencode/hooks/skill-verification-guard.ts 2>&1
{
  "session_id": "debug",
  "tool_name": "Edit",
...
**Check `.opencode/settings.json`:**

```bash
cat .opencode/settings.json | jq '.hooks.UserPromptSubmit'
cat .opencode/settings.json | jq '.hooks.PreToolUse'
```
        ...
        "UserPromptSubmit": [
            {
                "hooks": [
                    {
                        "type": "command",
                        "command": "$CLAUDE_PROJECT_DIR/.opencode/hooks/skill-activation-prompt.sh"
                    }
                ]
            }
        ]
...
```bash
head -1 .opencode/hooks/skill-activation-prompt.sh
```
...
```bash
# UserPromptSubmit
time echo '{"prompt":"test"}' | npx tsx .opencode/hooks/skill-activation-prompt.ts

# PreToolUse
time cat <<'EOF' | npx tsx .opencode/hooks/skill-verification-guard.ts
