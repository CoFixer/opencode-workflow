---
name: develop-skills
description: Creating and managing OpenCode skills, hooks, and skill-rules configuration
---
...
Comprehensive guide for creating and managing skills in OpenCode with auto-activation system, following Anthropic's official best practices including the 500-line rule and progressive disclosure pattern.
...
- **File**: `.opencode/hooks/skill-activation-prompt.ts`
- **Trigger**: BEFORE OpenCode sees user's prompt
- **Purpose**: Suggest relevant skills based on keywords + intent patterns
- **Method**: Injects formatted reminder as context (stdout → OpenCode's input)
- **Use Cases**: Topic-based skills, implicit work detection

### Configuration File

**Location**: `.opencode/skills/skill-rules.json`
...
echo '{"session_id":"test","prompt":"your test prompt"}' | \
  npx tsx .opencode/hooks/skill-activation-prompt.ts
```
...
- `false` only for universal OpenCode skills
...
- Exit code 2 from hook, stderr → OpenCode
- OpenCode sees message and must use skill to proceed
...
- Reminder injected before OpenCode sees prompt
- OpenCode is aware of relevant skills
...
```
.opencode/skills/backend-dev-guidelines/
...
```
.opencode/skills/error-tracking/
...
- [ ] Skill file created in `.opencode/skills/{name}/SKILL.md`
...
2. Add entry to `.opencode/skills/skill-rules.json` (if using legacy)
...
```bash
# UserPromptSubmit
echo '{"prompt":"test"}' | npx tsx .opencode/hooks/skill-activation-prompt.ts
```
...
- `.opencode/skills/skill-rules.json` - Master configuration (legacy)
- `.opencode/hooks/state/` - Session tracking
- `.opencode/settings.json` - Hook registration

**Hooks:**

- `.opencode/hooks/skill-activation-prompt.ts` - UserPromptSubmit

**All Skills:**

- `.opencode/skills/*/SKILL.md` - Skill content files
- `.opencode/skills/*/resources/*.md` - Reference files

**Project Examples:**

- `.opencode/skills/backend-dev-guidelines/` - Comprehensive multi-file skill
- `.opencode/skills/error-tracking/` - Single-file guardrail skill
- `.opencode/skills/route-tester/` - Testing-focused skill
