...
Task(
  ...
  prompt='Review refactored [module]. Verify improved structure, consistent patterns, and no broken functionality.'
)
```
...
Task(
  ...
  description='Fix errors after refactoring',
  prompt='Fix TypeScript compilation errors after refactoring [module]. Focus on import paths and type definitions.'
)
```
...
Task(
  ...
  prompt='Update documentation for [module] after refactoring. Reflect new file structure, component organization, and patterns.'
)
```
...
**Delegate when:**
- Refactoring complete and needs validation
- TypeScript errors need systematic resolution
- Documentation needs comprehensive updates

**Do NOT delegate:**
- Dependency tracking (core responsibility)
- File moving and reorganization (core responsibility)
- Import path updates (core responsibility)
- Component extraction (core responsibility)
